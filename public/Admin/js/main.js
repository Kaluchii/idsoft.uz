...
$(document).ready(function () {
    ...
    $('.menu .item a').each(function () {
        if ($(this).attr('href') == '/' + doc[1] + '/' + doc[2] + '/' + doc[3]) {
    ...
    $('.load-price').on('click', function () {
        ...
        deffered.done(function (data) {
            if (data.status == "OK") {
                ...
                $('.global-save').click();
                setTimeout(function () {
                        $('.status-panel').addClass('success').text('Прайс-лист загружен');
                    },
                    1000);
            }
        });
    ...
    $('.file-input').each(function(){
        var fileinput = $(this);
        fileinput.prepend('<i class="remove">x</i>');
        fileinput.find('i.remove').on('click',function(e){
            e.preventDefault();
            $(this).next('.preview').attr('src','');
            $(this).closest('.drag-n-drop').find('.img-hide-block').each(function(e){
                $(this).find('input').val('');
            });
        });
    });
    ...
    $('textarea').each(function(){
        if(!$(this).hasClass('no-trumbowyg')){
            $(this)
                .trumbowyg(customizedButtonPaneTbwOptions)
                .on('dblclick', function () { $(this).trumbowyg(customizedButtonPaneTbwOptions);});
        }
    });
    ...
    $('body').on('click', function () {
        $('.status-panel').removeClass('success').removeClass('alert');
    });
});
